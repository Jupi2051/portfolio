...
export type ValorantConstraints = {
...
export const EMPTY_CONSTRAINTS: ValorantConstraints = { mustPair: [], mustSplit: [] }

/** Fetches must-pair / must-split constraints between the currently active players. */
export function useValorantConstraints() {
  const trpc = useTRPC()
  return useQuery(trpc.valorantRandomizer.constraints.listActive.queryOptions())
}
