...
function TaskBarApp(Props: PropTypes) {
  ...
  return (
    <motion.button
      className={cn(
        "w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center relative select-none sm:px-8 group",
    ...
    >
      <motion.img
        ...
        src={Props.Icon}
        className={cn(
          "max-w-6 max-h-9 sm:max-w-10 sm:max-h-10 pointer-events-none",
      ...
      <motion.span
        className={cn(
          "absolute block bottom-0 -translate-y-full sm:-translate-y-px sm:top-full sm:bottom-0 left-1/2 -translate-x-1/2 bg-[#93909f] rounded-md h-[2.9px] sm:h-[3.34px] -mt-px border-transparent pointer-events-none",
          {
