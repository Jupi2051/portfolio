...
const createValorantConstraint = protectedProcedure
  .input(
    z.object({
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const existing = await ctx.prisma.valorantPlayerConstraint.findFirst({
      where: {
        playerAId,
        playerBId,
      },
    ...
    return ctx.prisma.valorantPlayerConstraint.create({
      data: { type: input.type, playerAId, playerBId },
    })
  })

export default createValorantConstraint
