 ...
 */
const applyValorantConstraintSides = protectedProcedure
  .input(
    z.object({
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    await ctx.prisma.$transaction([
      ctx.prisma.valorantPlayerConstraint.deleteMany({
        where: {
      ...
      ...pending.map((constraint) =>
        ctx.prisma.valorantPlayerConstraint.create({ data: constraint }),
      ),
    ])
...
export default applyValorantConstraintSides
