...
const refreshAllRivalsPlayers = protectedProcedure.mutation(async ({ ctx }) => {
  const players = await ctx.prisma.rivalsPlayer.findMany({
    select: { id: true, discordId: true, displayName: true },
    orderBy: { order: "asc" },
  })

  console.log(`[rivals players] refreshing all ${players.length} player(s)...`)
  ...
  for (const player of players) {
    try {
      ...
      console.log(`[rivals players] refreshed ${result.displayName} (id=${result.id})`)
      updated++
    } catch (error) {
      console.error(`[rivals players] failed to refresh ${player.displayName}:`, error)
      failed.push(player.displayName)
    }
  }

  console.log(
    `[rivals players] refresh all complete: ${updated} updated, ${failed.length} failed${
...
export default refreshAllRivalsPlayers
